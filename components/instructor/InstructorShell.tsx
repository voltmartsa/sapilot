...
const NAV = [
  { href: "/instructor", label: "My students" },
  { href: "/instructor/bookings", label: "Bookings" },
  { href: "/instructor/groundschool", label: "Groundschool" },
  { href: "/instructor/logbook", label: "Logbook" },
...
export default function InstructorShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto flex max-w-7xl gap-0 px-0 sm:px-4 lg:px-6 print:block print:max-w-none print:px-0">
      <aside className="hidden w-56 shrink-0 border-r border-line py-8 pr-4 md:block print:hidden">
        <p className="px-3 text-[11px] font-semibold uppercase tracking-[0.2em] text-gold-600">
          Instructor portal
        </p>
        <nav className="mt-3 space-y-1">
          {NAV.map((item) => {
            const active = item.href === "/instructor" ? pathname === "/instructor" : pathname.startsWith(item.href);
            return (
              <Link
                key={item.href}
                href={item.href}
      ...
      <div className="min-w-0 flex-1 py-8 md:pl-8 print:py-0 print:pl-0">
        <nav className="mb-6 flex gap-1 overflow-x-auto px-4 sm:px-0 md:hidden print:hidden">
          {NAV.map((item) => {
            const active = item.href === "/instructor" ? pathname === "/instructor" : pathname.startsWith(item.href);
            return (
              <Link
